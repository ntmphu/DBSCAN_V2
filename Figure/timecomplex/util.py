...
def compute_z_interval(j, n, d, O_obs, eps, neps, a, c, zk, minusO, x_zk):
  ...
  for j in O_obs:
    ...
    for k in range(len(A1_sparse)):
      delta = q1[0][k][0]*q1[0][k][0] - 4*p1[0][k][0]*t1[0][k][0]
      if - 1e-10 <= delta <= 1e-10:
        if p1[0][k][0] < 0:
          continue
        elif p1[0][k][0] > 0:
          if - 1e-20 <= zk[0][0] - (-q1[0][k][0]/(2*p1[0][k][0])) <= 1e-20:
            continue
          else:
            print("error1")
      elif delta > 0:
        z_1 = (-q1[0][k][0] - np.sqrt(delta))/(2*p1[0][k][0])
        z_2 = (-q1[0][k][0] + np.sqrt(delta))/(2*p1[0][k][0])
        ...
        interval.append([(z_1, z_2)])
        #print(f"single interval ({z_1}, {z_2})")
      else:
        if p1[0][k][0] < 0:
          interval.append([(np.NINF, np.inf)])
        else:
          print("error2")
  #   yTA1y = (x_i - x_j)^2  > eps^2
    for k in range(len(A2_sparse)):

        delta = q2[0][k][0]*q2[0][k][0] - 4*p2[0][k][0]*t2[0][k][0]
        if - 1e-10 <= delta <= 1e-10:
          if p2[0][k][0] < 0:
            continue
          elif p2[0][k][0] > 0:
          ...
          z_1 = (-q2[0][k][0] - np.sqrt(delta))/(2*p2[0][k][0])
          z_2 = (-q2[0][k][0] + np.sqrt(delta))/(2*p2[0][k][0])
          if z_1 > z_2:
            z_2, z_1 = z_1, z_2

          interval.append([(-float('inf'), z_1), (z_2, float('inf'))])
          #print(f"union (-inf, {z_1}), ({z_2}, inf)")
        else:
          if p2[0][k][0] < 0:
            interval.append([(np.NINF, np.inf)])
          else:
